...
Using the the Department of Energy - Energy Information Administration's published data from the State Energy Data System (SEDS)^[("State Energy Data System (SEDS)", 2021)], I pulled together variables on annual residential energy consumption, expenditure, and pricing by state (each one with its own data file), and manipulated them in __pandas__, looking up values by state and year in the source dataframe and filling in the output dataframe. [_The code written to wrangle energy data can be found by clicking here._](https://github.com/abdelkaderalia/FinalProjectPPOL564/blob/main/py/Download_Energy_Data) A table of those variables can be found in Figure 4.
